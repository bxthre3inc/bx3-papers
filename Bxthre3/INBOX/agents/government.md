# Government Affairs & Defense Contracts Lead — INBOX

**Role:** Government Affairs & Defense Contracts Lead — Agentic Government Department
**Reports to:** Navigate (VP Ventures), Anchor (CRO), Atlas (COO)
**Meeting Cadence:** Daily standup within Government at 8:15 AM
**Status:** ✅ ACTIVE — 2026-04-16
**Last Updated:** 2026-04-16 08:15 UTC

---

## Strategic Shift: ESTCP Exited

**bx3 decision (INBOX.md 2026-04-08):** ESTCP entirely cancelled. All obligations void.

### Implications — ACT NOW
| Item | Prior Status | New Status |
|------|-------------|------------|
| ESTCP Phase 2 sensor deployment | 6 days overdue | **CANCELLED** |
| ESTCP portal data flow | Active | **ABANDONED** |
| Airtable ESTCP records | Active | **NO LONGER RELEVANT** |
| ESTCP tasks in WORK_QUEUE | Active | **REMOVE** |
| Raj's government standup template | ESTCP-dependent | **REMOVE DEPENDENCY** |

---

## Active Grants Pipeline — Verified 2026-04-16

### P0/P1 — NONE ACTIVE

No grants with confirmed deadlines within 14 days.

### P2 — Active Opportunities

| Grant | ID | Agency | Deadline | Amount | Fit | Owner | Status |
|---|---|---|---|---|---|---|---|
| **NSF SBIR Phase I** | RAIN-001 | NSF | Rolling | $275K | HIGH — digital ag | maya | ✅ OPEN — most immediately actionable |
| **ARPA-E OPEN 2026** | ARPA-E-001 | DOE | FA Deadline TBD | $250K–$10M | HIGH — energy/ag | casey | ✅ ACTIVE — DE-FOA-0003567 |
| **USDA REAP** | REAP-001 | USDA RD | Varies [VERIFY] | $300K–$1M | MED-HIGH | maya | 🟡 VERIFY |
| **CIG Colorado** | FED-CO-003 | USDA NRCS | TBD by CO NRCS | Varies | HIGH — soil/water | casey | 🟡 CONTACT NRCS CO |
| **USDA SBIR Phase I FY2026** | IRR-002 | USDA NIFA | FY2026 NOT RELEASED | $125K–$181.5K | HIGH — irrigation | maya | 🔴 TRACK — FY2025 closed 9/17/24 |

---

## ARPA-E OPEN 2026 — Confirmed Active

| Item | Status |
|------|--------|
| FOA | DE-FOA-0003567 |
| Posted | February 13, 2026 |
| Full Application Deadline | **TBD** — currently unconfirmed |
| Amount | $250K–$10M |
| Fit | HIGH — energy/ag tech, precision irrigation |
| Owner | casey |
| Source | ARPA-E eXCHANGE (arpa-e-foa.energy.gov) — verified 2026-04-14 |

**Action:** Monitor ARPA-E eXCHANGE. When FA deadline is announced, assess if ≥14 days to prepare.

---

## USDA SBIR FY2026 — NOT YET RELEASED

| Item | Status |
|------|--------|
| FY2025 Status | Closed September 17, 2024 |
| FY2026 NOFO | **NOT POSTED** |
| Source | NIFA (nifa.usda.gov/sbir) — verified 2026-04-14 |

**Action:** Monitor NIFA monthly for FY2026 release.

---

## CIG Colorado — Action Required

| Item | Status |
|------|--------|
| Grant ID | FED-CO-003 |
| Deadline | TBD by CO NRCS |
| Priority | P2 |
| Assigned | casey |
| NRCS CO Contact | Sarah Stoeber, SLVDRG — 719-589-6099 |

**Action:** Contact Sarah Stoeber this week to confirm 2026 State CIG Classic deadline.

---

## Cash Position Context

| Item | Value |
|------|-------|
| Cash | ~$130,450 [VERIFY against financial docs] |
| Burn rate | ~$130K/week [VERIFY] |
| Runway | ~1 week [VERIFY] |
| Bridge required | $400K by July 1 (84 days) |

**Implication:** NSF SBIR ($275K rolling) is the most immediately actionable opportunity.

---

## Future Pipeline (Post-ESTCP)

| Opportunity | Agency | Status |
|-------------|--------|--------|
| DIU opportunities | Defense Innovation Unit | SCAN QUEUED |
| State ag tech initiatives | Colorado OIT | PROSPECT |
| CDPHE water grants | CDPHE | RESEARCH |
| ARS partnerships | USDA ARS | PROSPECT |
| Land-grant partnerships | CSU / Adams State | OUTREACH |

---

## Priority Actions This Week

| # | Action | Owner | Status |
|---|---|---|---|
| 1 | NSF SBIR Phase I — prepare proposal narrative | maya | 🟡 TODO |
| 2 | Contact CO NRCS (Sarah Stoeber 719-589-6099) — confirm CIG Colorado 2026 deadline | casey | 🟡 TODO |
| 3 | Verify USDA REAP current round deadline with USDA RD state office | maya | 🟡 TODO |
| 4 | Monitor ARPA-E eXCHANGE for FA deadline announcement | casey | ⏳ WAITING |
| 5 | Monitor NIFA monthly for FY2026 SBIR NOFO release | maya | 🔴 TRACK |

---

## Reports To

| Role | Agent | Responsibility |
|------|-------|----------------|
| VP Ventures | Navigate | Strategic direction |
| CRO | Anchor | Revenue/partnerships |
| COO | Atlas | Operations alignment |
| Grant Strategist | maya | Grants infrastructure |
| Marketing Lead | casey | Grant writing + outreach |

---

## Department Structure

- **INBOX:** `Bxthre3/INBOX/agents/government.md`
- **Department INBOX:** `Bxthre3/INBOX/departments/government.md`
- **Daily Standup:** `government-standup-YYYY-MM-DD.md`
- **Reports to:** Navigate, Anchor, Atlas

---

*Government Affairs — Updated 2026-04-16 08:15 UTC*