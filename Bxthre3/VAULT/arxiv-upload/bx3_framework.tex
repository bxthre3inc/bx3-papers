...
\lhead{\small Preprint --- arXiv cs.AI / cs.SE}
\cfoot{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

% --- Hyperref metadata for arXiv indexing ---
\hypersetup{
    colorlinks=true,
    linkcolor=humanblue,
    citecolor=humanblue,
    ...
    pdfkeywords={BX3 Framework, Purpose Layer, Bounds Engine, Fact Layer, deterministic systems, upstream accountability, agentic systems, AI governance},
    pdfcreator={pdfLaTeX},
...
\title{
    \vspace{-1cm}
    {\Large \textbf{The BX3 Framework:}}\\[0.4em]
    {\large Implementation Protocols for\\
    Accountable Autonomous Systems}\\[0.8em]
    {\normalsize \textit{Three Functional Layers. Five Named Protocols. Guaranteed Upstream Accountability.}}
}

\author{
    \textbf{Jeremy Blaine Thompson Beebe}\\[0.2em]
    \textit{Independent Researcher}\\[0.3em]
    \textit{Submitted for review --- arXiv cs.AI / cs.SE}\\[0.3em]
}
...
We demonstrate, on proprietary internal systems, that when these three functional layers are clearly separated and their properties enforced by design, systems become simultaneously more capable and less complex. We further argue that the Bounds Engine is most powerfully employed not as a replacement for the Fact Layer but as a tool for designing, accelerating, and improving it. We show that the BX3 Framework is not merely a software engineering principle but a universal structural pattern observable across law, medicine, autonomous systems, organizational design, and biological cognition. The framework is currently under active implementation in a active agentic business enterprise deployment, with field trials planned for June 2026 --- providing an initial empirical test across all five architectural pillars. A companion engineering specification --- the BX3 Protocol --- is in preparation and will provide normative, certifiable implementation standards derived from this framework.
...
The framework is currently under active implementation in an active agentic business enterprise deployment, with field trials planned for June 2026 that will provide an initial empirical test across all five architectural pillars. A companion engineering specification --- the BX3 Protocol --- is in preparation and will provide normative, certifiable implementation standards derived from this theoretical foundation.
...
\begin{center}
\begin{tikzpicture}[
    box/.style={rectangle, rounded corners=6pt, minimum width=10.5cm, minimum height=1.2cm,
                text centered, font=\bfseries\small, draw, thick},
    subbox/.style={font=\small\itshape, text=gray},
    arrow/.style={-{Stealth[length=8pt]}, thick},
    label/.style={font=\small\itshape, text=gray}
]

\node[box, fill=humanblue!15, draw=humanblue] (human)
    {LAYER 1: PURPOSE \quad \textmd{Intent | Judgment | Accountability}};
\node[subbox, below=0.05cm of human] (humansub) {\textit{Human Root Mandate --- accountable actor required}};

\node[box, fill=aigreen!15, draw=aigreen, below=0.9cm of human] (ai)
    {LAYER 2: BOUNDS ENGINE \quad \textmd{Analysis | Bounded Reasoning | Proposal (Limbless)}};
\node[subbox, below=0.05cm of ai] (aisub) {\textit{Can propose --- cannot execute without Fact Layer approval}};

\node[box, fill=detgray!15, draw=detgray, below=0.9cm of ai] (det)
    {LAYER 3: FACT LAYER \quad \textmd{Physical Firewall | Hard Constraint | Forensic Ledger}};
\node[subbox, below=0.05cm of det] (detsub) {\textit{100\% deterministic --- hard-blocks any constraint violation}};

\draw[arrow, humanblue] (human.south) -- (ai.north)
    node[midway, right=6pt, label] {directs \& sets SLO};
\draw[arrow, aigreen] (ai.south) -- (det.north)
    node[midway, right=6pt, label] {proposes action};
\draw[arrow, detgray, bend right=65] (det.west) to
    node[midway, left=4pt, label] {forensic ledger \& escalation} (human.west);
\draw[arrow, detgray!80, bend left=65] (det.east) to
    node[midway, right=4pt, label] {hard-blocks violations} (ai.east);
...
\draw[arrow, bend left=40, dashed] (hub1.north) to
    node[midway, left=3pt, label] {Bailout escalation} (root.south west);
...
\hrule
\vspace{0.5em}
\noindent\small\textit{This preprint has not undergone peer review. Comments and correspondence are welcomed.}
