...
Wefurtherarguethatthe BX3Frameworkisnotanovelinventionbutasynthesisofwellestablished principles — separation of concerns \citep{dijkstra1982}, sociotechnical systems theory \citep{trist1981}, control theory \citep{wiener1948}, and human-in-the-loop design \citep{bansal2019} — applied to the specific and urgent challenge of AI integration in the current technological moment.
...
\draw[constraintarrow] (fact.east)--++(2cm,0)|-(bounds.east) node[pos=0.25,right,xshift=0.2cm,labeltext,align=left]{\textbf{Hard Block}\\ ootnotesize Constraint Violations};
\draw[escalationarrow] (fact.west)--++(-2cm,0)|-(purpose.west) node[pos=0.25,left,xshift=-0.2cm,labeltext,align=right]{Forensic Ledger\& Escalation};
...
The \bxthree{} defines not only what each layer must do but what crosses each layer boundary. Well-defined interfaces are the mechanism by which layer isolation is maintained in practice.
...
The Sandbox Gate requires the Bounds Engine to model the projected outcome of any proposed action in a digital twin environment before physical actuators are unlocked \citep{nist2023}.
...
\subsection{Biological Cognition} Notably, the BX3 Framework mirrors the layered architecture of biological cognition \citep{kahneman2011}.
...
Separation of Concerns \citep{dijkstra1982}: Dijkstra’s foundational principle that different aspects of a problem should be handled by different components is extended here to explicitly include human and AI roles alongside traditional software components — a dimension the original principle did not address.

Sociotechnical Systems Theory \citep{trist1981}: The Tavistock Institute’s work on the joint optimization of social and technical systems anticipated the importance of explicitly modeling human roles within technical systems rather than treating them as external to the design.

Cybernetics and Control Theory \citep{wiener1948}: Wiener’s foundational work on feedback systems and the sense-process-act loop provides the theoretical basis for understanding how deterministic control systems interact with adaptive agents and human oversight.

Human-in-the-Loop Design \citep{bansal2019}: The extensive literature on HITL systems provides empirical grounding for the claim that human oversight remains essential in AI-assisted systems, particularly for safety-critical applications.
...
The most closely related contemporary work is the intelligent sociotechnical systems (i STS) framework proposed by Xu and Gao \citep{russell2019}, which similarly extends traditional STS theory to address the demands of AI integration and proposes a hierarchical human-centered AI approach.
...
The NIST AI Risk Management Framework \citep{leveson2011} and ISO/IEC 42001 \citep{brooks1987} provide governance and risk management structures for AI systems but do not prescribe an architectural model for how AI, deterministic, and human components should be separated within a system.
...
The most closely related contemporary work on escalation and oversight is Tiered Agentic Oversight (TAO) \citep{kim2025}, a hierarchical multi-agent system for healthcare safety that routes tasks through specialized agent tiers based on complexity and escalates unresolved cases upward through the hierarchy.
...
Alenezi \citep{alenezi2026} argues that agentic systems must be designed to fail safely, with key architectural patterns including clear escalation paths to human oversight, fallback deterministic workflows, and sandbox-first execution before commitment.
...
Acomplementaryformal-verificationapproachisdemonstratedbythe Lean-Agent Protocol \citep{rashie2026}, which permits agentic execution only when a proof kernel can verify that a proposed action satisfies precompiled regulatory axioms, functioning as a deterministic gateway in financial systems.
...
Russell’s foundational work on AI control \citep{xu2024} and Leveson’ssystemssafetyengineering\citep{nist2023}providefurthergroundingforthesafety-critical arguments made in Sections 5 and 6.

Brooks’ seminal observation that there is no single silver bullet in software engineering \citep{iso2023} anticipates the central claim of this paper — that AI is not a universal solution but one bounded layer in a properly structured system.
